-- Fix: Convert RESTRICTIVE RLS policies to PERMISSIVE on all tables
...
CREATE POLICY "Users can view their own bulk analyses" ON public.bulk_analyses
  FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own bulk analyses" ON public.bulk_analyses
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own bulk analyses" ON public.bulk_analyses
  FOR UPDATE TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their own bulk analyses" ON public.bulk_analyses
...
CREATE POLICY "Users can view their own analyses" ON public.startup_analyses
  FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own analyses" ON public.startup_analyses
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own analyses" ON public.startup_analyses
  FOR UPDATE TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their own analyses" ON public.startup_analyses
  FOR DELETE TO authenticated USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view their own comparisons" ON public.comparison_analyses
  FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can create their own comparisons" ON public.comparison_analyses
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can delete their own comparisons" ON public.comparison_analyses
  FOR DELETE TO authenticated USING (auth.uid() = user_id);
...
-- admin_users - keep deny-all but as permissive (it uses false so effect is same)
DROP POLICY IF EXISTS "Deny all access to admin_users" ON public.admin_users;

CREATE POLICY "Deny all access to admin_users" ON public.admin_users
